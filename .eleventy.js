...
// Date filter for formatting with error handling
...
function postDateKannada(date) {
  try {
    if (!date) return 'ದಿನಾಂಕ ಸೆಟ್ ಆಗಿಲ್ಲ';
    const dateObj = new Date(date);
    if (isNaN(dateObj.getTime())) return 'ಅಮಾನ್ಯ ದಿನಾಂಕ';
    return dateObj.toLocaleDateString('kn-IN', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
...
// ISO Date filter for datetime attributes with error handling
...
// Simple date for display with error handling
...
module.exports = function(eleventyConfig) {
  ...
  // Create news collections with safe date handling
